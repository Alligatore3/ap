...
<head>
    ...
    <script src="js/mep-feature-markers.js"></script>
...
<div id="<?php echo substr($tone[0], 0, -4);?>" class="track">
	<span class="date">.date( <?php echo '<font class="normal">'.$tone[1].'</font>'; ?> )</span>
	<span class="descr">.descr( <?php echo '<font class="normal">'.$tone[2].'</font>'; ?> )</span>
	<audio class="aud <?php if ($tone[3]) { echo $tone[3];} ?>" src="assets/audio/<?php echo $tone[0]; ?>" type="audio/mp3" controls="controls"></audio>
</div>

<?php
		endforeach;
		echo '<font class="normal">'. count($audio) .'</font>';
?>
	</div>
...
?>
	...
	<div class="video col-md-12 col-sm-12 text-center m-b-100">
		<?php foreach ($video as $vid): ?>
			<video class="vid center" src="assets/video/<?php echo $vid[0]; ?>" poster="assets/images/cover/<?php echo $vid[1] ?>" preload="true"  controls="controls"></video>
		<?php endforeach; ?>
	</div>
	<script>
		//var w = $(document).width();
		$('.aud').mediaelementplayer({
			audioWidth: 750,
			features: ['playpause','current','progress','duration','markers','volume'],
